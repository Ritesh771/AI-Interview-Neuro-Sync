  ...
  constructor(doc, logger) {
    ...
    this.setLocalState = (state) => {
      ...
      if (state === null) {
        ...
        (_a = this.room) == null ? void 0 : _a.presence.update(__spreadProps(__spreadValues({}, this.states.get(this.clientID)), {
          [this.YJS_STATE]: null,
      ...
      const newState = __spreadProps(__spreadValues({}, oldState), { [this.YJS_STATE]: state, origin: "set local state" });
    ...
    this.onUpdate = (event) => {
      ...
      this.states.set(clientID, __spreadValues(__spreadValues({}, this.states.get(clientID) || {}), event.data));
  ...
  initializePresences() {
    this.room.presence.get((presences) => {
      ...
      presences.forEach((presence) => {
        ...
        this.states.set(clientID2, __spreadValues({}, presence.data));
      });
      const { clientID } = this.doc;
      this.room.presence.update(__spreadProps(__spreadValues({
        clientID,
        [this.YJS_STATE]: {}
...
function getUpdatesHistory(channelName, apiKey) {
  return __async(this, null, function* () {
    try {
      const response = yield fetch(`https://io.superviz.com/yjs/${channelName}:yjs-provider`, {
        headers: {
          "sv-api-key": apiKey
        }
      });
...
var SuperVizYjsProvider = class extends ObservableV2 {
  constructor(doc, opts) {
     ...
     * @returns {void}
     */
    this.fetch = () => __async(this, null, function* () {
      this.logger.log("Fetching the document");
      this._synced = false;
      const updates = yield getUpdatesHistory(this.roomId, this.ioc["client"]["apiKey"]);
      this._synced = true;
